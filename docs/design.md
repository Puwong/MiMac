...
团队这边现在功能不完善，应该在relationship表加一个字段表示状态，增加管理员审核和用户同意这两个功能，现在是管理员直接添加
...
有一些非系统需要的常量就不要写在base.py里面，写在my_app.common.constant里面就不会造成递归饮用问题。
  - 比如ALG_DIR和USER_DIR
...
权限问题现在统一由UserService下的hack_alert接管处理
  - 这边可能对用户的hack行为进行记录，如果一天超过三次直接pending他
    - 这个penging我好想改成freeze。。。开始的时候脑子进水了吧这么设计

潜在问题：文章分块，顺序可能是不确定的，最好可以定义为按照id顺序获取
