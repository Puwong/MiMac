...
messagebatch可以绑定imageid

有一个最高优先级的东西：在图像那里加上search。作为一个管理系统，增删改查是最基本的。

团队功能不完善，应该在relationship表加一个字段表示状态，增加管理员审核和用户同意这两个功能，现在是管理员直接添加

消息功能不完善，缺少删除会话，缺少对于未读条数的提醒。

文章功能不完善，可以选择是否公开，文章可以加标签。
...
还有my_app.common里面的一些方法不是通用方法，可能需要移动到ImageService里面

录屏的时候发现两个bug
- 文件名很长的时候前端显示会有问题（说到这个，在存入到数据库前也需要校验，否则从中间截断可能改变后缀攻击服务器）
- 连续上传图片会导致猫狗识别报错。(我怕是知道发生什么了，训练函数没有写好但是已经运行了，覆盖了正确的权重)

有个问题，article的正文。。。怎么编辑才能保证格式，各种关键字怎么处理（引号、sql关键字、前端关键字）。
- 在textarea里面\r\n是换行，再次放进去还是换行美滋滋。但是正文`<p>`显示不能换行了，待解决。（暂时用`<pre>`标签解决了，但是有一个框。。。略丑，有没有代替的方案。。？）

权限问题现在统一由UserService下的hack_alert接管处理
  - 这边可能对用户的hack行为进行记录，如果一天超过三次直接pending他（可以用mongo）
    - 这个penging我好想改成freeze。。。开始的时候脑子进水了吧这么设计

## 已经解决的问题（重构的时候注意应用这些新的策略）

即使忽略RESTful不规范的问题，现在还面临post后（或者delete的get）在当前方法中render导致不能刷新的问题
  - 问题已经解决了，redirect后url也会变，然后在重定向目的地的post下return get就好

template里面的每一块对应的场景写清楚，传入的时候做确定
...
- Logout (done)
- permission (系统管理员，用户)
- password encode (done)
  - 现在的salt是死的，应该给每一个用户单独一个salt
- reset password (done)
- FindPasswordByEmail (abandon)
- OAuth2 (abandon)
...
- 上传图片 (doing)
  - 选择文件所属的任务类型 (done)
    - 要做成可配置的，每一个抽象下属可增加可删除的细分需求 (done)
      - （做成可配置的容易出问题，后期可以考虑迁移的问题） (pending)
    ...
  - 上传 (done)
  - 更新db，放入图像，关联用户，获得图像ID （文件处于待标注状态）(done)
  - 在该用户的文件夹下放置文件，uri为image_id.[type]  (done)
  - 为文件添加标注文件，uri为image_id.label（本质是json） (done)
  - 异步任务开始进行预标注（这里有两种实现策略） (done)
    - 一个是本地跑 (done)
      - 前提是单张图片本地预标注的时间控制在1分钟内 (done)
...
- 算法
  ...
  ```python
    ...
    ```
- 行为(doing)
  - 这里要加一个针对毕设演示的后门，训练成本太高了 (done)
    - 做成了基于文件名判断的（本来可以做成基于md5的，事实上，，，最后演示的时候大家根本没注意状态的自动变化和结果是否正确。。。直接放个随机值也可以蒙混过关。。。。。）
  - 标注(done)
...
## 前端 (abandon)
- 我的前端跟`屎`一样我没啥想说的 （现在总算能看了。。。不再像屎一样了）
  - 本来是想用iview的，暂时不想了，随缘吧 （pending）
  - 写完论文后用 bootstrap 看情况美化一下吧 (done)
  - bootstrap真香。不过感觉还是用js装饰一下会好无数倍 (todo)
