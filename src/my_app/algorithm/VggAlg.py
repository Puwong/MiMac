...
class VggAlg(object):
    ...
    def train_top_model(self):
        if not os.path.isfile('bottleneck_features_train.npy') or \
                not os.path.isfile('bottleneck_features_validation.npy'):
            self.save_bottlebeck_features()
        train_data = np.load(open('bottleneck_features_train.npy','rb'))
        ...
        self.model.save_weights(top_model_weights_path)
